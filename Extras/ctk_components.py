...
from window_position import center_window, place_frame

current_path = pathlib.Path(__file__).resolve()
CURRENT_PATH = current_path.parents[1]
ICON_DIR = CURRENT_PATH / "icons"

"""
if getattr(sys, 'frozen', False):
    ICON_DIR = pathlib.Path(sys._MEIPASS) / "src" / "icons"
else:
    ICON_DIR = CURRENT_PATH / "src" / "icons"
"""
ICON_PATH = {
    "close": (ICON_DIR / "close_black.png", ICON_DIR / "close_white.png"),
    "images": [ICON_DIR / f"image{i}.jpg" for i in range(1, 4)],
    "eye1": (ICON_DIR / "eye1_black.png", ICON_DIR / "eye1_white.png"),
    "eye2": (ICON_DIR / "eye2_black.png", ICON_DIR / "eye2_white.png"),
    "info": ICON_DIR / "info.png",
    "warning": ICON_DIR / "warning.png",
    "error": ICON_DIR / "error.png",
    "left": ICON_DIR / "left.png",
    "right": ICON_DIR / "right.png",
    "warning2": ICON_DIR / "warning2.png",
    "loader": ICON_DIR / "loader.gif",
    "icon": ICON_DIR / "icon.png",
    "arrow": ICON_DIR / "arrow.png",
...
class CTkCarousel(ctk.CTkFrame):
    def __init__(self, master: any, img_list=None, width=None, height=None, img_radius=25, **kwargs):
        ...
        pywinstyles.set_opacity(self.previous_button.winfo_id(), color=self.button_bg[0])
        ...
        pywinstyles.set_opacity(self.next_button.winfo_id(), color=self.button_bg[0])
...
class CTkLoader(ctk.CTkFrame):
    def __init__(self, master: any, opacity: float = 0.8, width: int = 40, height: int = 40):
        ...
        pywinstyles.set_opacity(self.winfo_id(), value=opacity)
        self.grid_rowconfigure(0, weight=1)
        self.grid_columnconfigure(0, weight=1)
